...
export function UploadForm() {
  ...
  const mutation=useMutation({mutationFn:uploadRoute,onSuccess:({publicId,managementToken})=>setCreated({publicId,managementUrl:managementRouteUrl(publicId,managementToken,window.location.origin)}),onError:(value)=>setError(value instanceof Error?value.message:"Upload failed")});
  ...
  return <><form onSubmit={submit} className="mt-8 space-y-5 rounded-xl border border-slate-200 bg-white p-6 text-left shadow-sm">
    ...
    <section role="dialog" aria-modal="true" aria-labelledby="management-title" className="w-full max-w-xl rounded-2xl border border-amber-200 bg-white p-6 shadow-2xl sm:p-8">
      ...
      <h2 id="management-title" className="text-2xl font-bold text-slate-950">Save your management link</h2>
      <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-slate-700">
        <p>This private link gives you full control over this route. Keep it somewhere safe and do not share it publicly.</p>
        <p className="mt-3 font-semibold">Route Community does not use accounts, so if you lose this link, we cannot recover access to route management.</p>
      </div>
      <label className="mt-5 block text-sm font-semibold text-slate-800">Management link
      ...
      {!copyFailed?<button type="button" onClick={copyAndContinue} disabled={copied} className="mt-5 w-full rounded-lg bg-emerald-700 px-5 py-3 font-bold text-white disabled:bg-emerald-600">{copied?"Copied! Opening route…":"Copy management link and continue"}</button>:
      <div className="mt-5 rounded-lg border border-red-200 bg-red-50 p-4">
        <p role="alert" className="text-sm text-red-800">Automatic copying was blocked. Select and copy the link above manually, then confirm below.</p>
        <label className="mt-3 flex items-start gap-2 text-sm font-medium text-slate-800"><input type="checkbox" checked={manualConfirmed} onChange={(event)=>setManualConfirmed(event.target.checked)} className="mt-1" />I have copied and safely saved the management link.</label>
        <button type="button" onClick={continueAfterManualCopy} disabled={!managementModalCanContinue(copied,copyFailed,manualConfirmed)} className="mt-3 w-full rounded-lg bg-emerald-700 px-5 py-3 font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">Continue to route</button>
      </div>}
    </section>
  </div>}</>;
}
